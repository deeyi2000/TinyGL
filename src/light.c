...
void glopMaterial(GLContext *c,GLParam *p)
{
  ...
  case GL_SHININESS:
    m->shininess=p[3].f;
    m->shininess_i = sll2int(sllmul(slldiv(m->shininess, int2sll(128)), int2sll(SPECULAR_BUFFER_RESOLUTION)));
    break;
...
void glopLightModel(GLContext *c,GLParam *p)
{
  ...
  case GL_LIGHT_MODEL_LOCAL_VIEWER:
    c->local_light_model=sll2int(p[2].f);
    break;
  case GL_LIGHT_MODEL_TWO_SIDE:
    c->light_model_two_side = sll2int(p[2].f);
    break;
  default:
...
void gl_shade_vertex(GLContext *c,GLVertex *v)
{
  ...
  for(l=c->first_light;l!=NULL;l=l->next) {
    ...
    if (sllvalue(l->position.v[3]) == sllvalue(int2sll(0))) {
      ...
      if (sllvalue(dist)>sllvalue(float2sll(1E-10f))) {
